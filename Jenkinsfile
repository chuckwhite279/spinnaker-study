node {
	...
	stage('Create tar.gz') {
       sh 'tar -czvf flexible-danger-microservice-chart.tar.gz -C helm flexible-danger-microservice'
       archiveArtifacts artifacts: 'flexible-danger-microservice-chart.tar.gz', fingerprint: true
	}
	stage('Push chart to S3') {
	    withAWS(region:'us-west-2', credentials:'spinnaker-admin-aws') {
          s3Upload(file:'flexible-danger-microservice-chart.tar.gz', bucket:'helm-charts-f2bba284-98d3-445b-9f04-a08c57b7d36f', path:"${JOB_NAME}/${BUILD_ID}/flexible-danger-microservice-chart.tar.gz")
        }
	}
